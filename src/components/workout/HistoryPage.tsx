...
export function HistoryPage() {
  const weekSessions = useMemo(() => getWeekHistory(), []);
  const allSessions = useMemo(() => loadHistory().sort((a, b) => b.date.localeCompare(a.date)), []);

  const weekDays = trainingDays.map((day) => {
    const session = weekSessions.find(s => s.dayIndex === day.dayIndex);
    return { day, session };
  });
  ...
  return (
    <div>
      ...
      {allSessions.slice(0, 20).map((session, i) => {
