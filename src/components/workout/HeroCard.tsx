...
export function HeroCard({ day, progress }: HeroCardProps) {
  ...
  return (
    <div className="bg-bg3 border border-border rounded-lg p-4 mb-4 relative overflow-hidden">
      <div className={`absolute top-0 left-0 right-0 h-[3px] ${colorMap[day.colorClass] ?? 'bg-primary'}`} />
      {day.dayLabel && <div className="text-[11px] text-muted-foreground mb-1 uppercase tracking-wider">{day.dayLabel}</div>}
