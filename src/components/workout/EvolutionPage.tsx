...
export function EvolutionPage() {
  const allDays = useMemo(() => trainingDays.filter(d => !d.isRest), []);
  const firstId = allDays[0]?.exercises[0]?.id || "";
  const [selectedExercise, setSelectedExercise] = useState(firstId);
  const [mode, setMode] = useState<"top" | "volume">("top");

  const topData = useMemo(() => {
    return getTopSetHistory(selectedExercise).map(e => ({
  ...
  const volData = useMemo(() => {
    return getVolumeHistory(selectedExercise).map(e => ({
      date: new Date(e.date + "T12:00:00").toLocaleDateString("pt-BR", { day: "numeric", month: "short" }),
      volume: Math.round(e.volume),
    }));
  }, [selectedExercise]);

  const sessions = useMemo(() => getExerciseSessions(selectedExercise).slice(0, 8), [selectedExercise]);

  const exerciseName = allDays.flatMap(d => d.exercises).find(e => e.id === selectedExercise)?.name || "";
  ...
  return (
    <div>
      ...
      {sessions.length > 0 && (
        <>
          <div className="text-[10px] font-semibold tracking-[0.12em] uppercase text-muted-foreground mb-[10px]">
            Sessões recentes (séries detalhadas)
          </div>
          <div className="space-y-2 mb-4">
            {sessions.map((s, i) => (
              <div key={i} className="bg-bg2 border border-border rounded-lg p-3">
